...
1) the ```public static void main(String[] args)``` is similar to the ```if name == main:``` statement in python
2) the for loop structure is different compared to the python for loop structure
3) the ```&&``` logical operator is equivalent to the  ```and``` operator in python
4) the ```%``` and the ```==``` operator is the same in java as in python 
5) the if and else if conditions require curly braces and not just indentation as with the python if and elif conditions
